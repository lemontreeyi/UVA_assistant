...
void calculate_location(float d[], float location[], float height)
{
    // 先将三维距离转化为xy平面投影上的距离
    // d_xy = sqrt(d**2 - dh**2)
    float d1 = (float)sqrt(d[0]*d[0] - (STATION1_Z - height)*(STATION1_Z - height));
    float d2 = (float)sqrt(d[1]*d[1] - (STATION2_Z - height)*(STATION2_Z - height));
    float d3 = (float)sqrt(d[2]*d[2] - (STATION3_Z - height)*(STATION3_Z - height));
    float d4 = (float)sqrt(d[3]*d[3] - (STATION4_Z - height)*(STATION4_Z - height));
    float e1[3] = {
