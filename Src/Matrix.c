...
{
    //转化到m为单位
    float tar_x = (float)(target_location[0] / 100);
    float tar_y = (float)(target_location[1] / 100);
    ...
    else
    {
        ...
        //转化到cm为单位
        //next_y
        next_location[1] = (int)((current_location[1] + 0.8 * ((target_location[1] - current_location[1] > 0)?1.0:-1.0) * sin)*100);
        //next_x
        next_location[0] = (int)((current_location[0] + 0.8 * ((target_location[0] - current_location[0] > 0)?1.0:-1.0) * cos)*100);
    }
}
